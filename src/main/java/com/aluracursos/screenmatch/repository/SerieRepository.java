...
public interface SerieRepository extends JpaRepository<Serie, Long> {
   ...
   //lamzamientos mas reciente de una serie, se utilizara JPQL
   @Query("SELECT s FROM Serie s JOIN s.episodios e GROUP BY s " +
         "ORDER BY MAX(e.fechaDeLanzamiento) DESC LIMIT 5")
   List<Serie> lanzamientosMasRecientes();

   // obtener numero de temporada
   @Query("SELECT e FROM Serie s JOIN s.episodios e WHERE s.id=:id AND e.temporada = :numeroTemporada")
   List<Episodio> obtenerNumeroDeTemporada(Long id, Long numeroTemporada);
